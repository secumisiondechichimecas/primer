    ...
    if (isAuthUser) {
      ...
      if (isCurrentCartItemAlreadyExists?.length > 0) {
        return NextResponse.json({
          success: false,
          message:
            "Material is already added in chart! Please add different material",
        });
      }
      ...
      if (saveProductToCart) {
        return NextResponse.json({
          success: true,
          message: "Material is added to chart !",
        });
      } else {
        return NextResponse.json({
          success: false,
          message: "failed to add the Material to chart ! Please try again.",
        });
      }
    } else {
      return NextResponse.json({
